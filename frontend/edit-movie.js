const updateMovie = async () => {
  ...
  dataObj.id = document.querySelector("th").id;
  document.getElementById("title").value === ""
    ? (dataObj.title = document.getElementById("title").placeholder)
    : (dataObj.title = document.getElementById("title").value);
  document.getElementById("director").value === ""
    ? (dataObj.director = document.getElementById("director").placeholder)
    : (dataObj.director = document.getElementById("director").value);
  document.getElementById("runtime").value === ""
    ? (dataObj.runtime = parseInt(
        document.getElementById("runtime").placeholder
      ))
    : (dataObj.runtime = parseInt(document.getElementById("runtime").value));
  document.getElementById("rating").value === ""
    ? (dataObj.rating = parseFloat(
        document.getElementById("rating").placeholder
      ))
    : (dataObj.rating = parseFloat(document.getElementById("rating").value));
  document.getElementById("description").value === ""
    ? (dataObj.description = document.getElementById("description").placeholder)
    : (dataObj.description = document.getElementById("description").value);
...
const editMovie = (param) => {
  fetch("http://localhost:3001/movies/get-one-movie", {
    ...
    .then((res) => {
      ...
      document.querySelector("th").id = data._id;
  ...
  fetch("http://localhost:3001/movies/get-all-movies")
    ...
    .then((res) => {
      document.querySelector("#movielist").innerHTML = res.map((e) => {
        return `<button onclick="editMovie('${e._id}')">${e.title}</button>`;
      });
    })
    .catch((error) => {
      console.log(error);
